...
static void resetPidAxisFilters(pidAxis_t* axis, float vaGain) {
    pt1FilterInit(&axis->velocityLpf, vaGain);
    pt1FilterInit(&axis->accelerationLpf, vaGain);
}

static void resetPidAxisParams(pidAxis_t* axis, const float vaGain)
{
    // at start only
    resetPidAxisFilters(axis, vaGain);
    axis->isStopping = true; // Enter starting (deceleration) 'phase'
    axis->integral = 0.0f;
}
...
void resetPositionControl(const gpsLocation_t *initialTargetLocation)
{
    ...
    for (unsigned i = 0; i < ARRAYLEN(ap.pidAxis); i++) {
        // disable filter until first call
        resetPidAxisParams(&ap.pidAxis[i], 1.0f);
    }
...
void autopilotInit(void)
{
...
#if USE_POS_HOLD_MODE // TODO - POSHOLD_TASK_RATE_HZ without poshold
    ap.upsampleGain = pt3FilterGain(UPSAMPLING_CUTOFF_HZ, HZ_TO_INTERVAL(POSHOLD_TASK_RATE_HZ)); // 5Hz
    ...
    for (unsigned i = 0; i < ARRAYLEN(ap.pidAxis); i++) {
        resetPidAxisFilters(&ap.pidAxis[i], vaGain);
    }
}
...
static void setTargetLocationAxis(const gpsLocation_t* newTargetLocation, axisEF_e efAxisIdx)
{
    if (efAxisIdx == LON) {
...
bool positionControl(void)
{
    ...
    if (gpsHasNewData(&gpsStamp)) {
        ...
        // first get lon and lat distances from current location (gpsSol.llh) to target location
        ...
        for (axisEF_e pidAxisIdx = 0; ARRAYLEN(ap.pidAxis); pidAxisIdx++) {
            ...
            const float axisDistance = gpsDistance.v[pidAxisIdx];
            ...
            pidAxis->integral += pidAxis->isStopping ? 0.0f : axisDistance * gpsDataInterval;
            ...
            const float velocity = (axisDistance - pidAxis->previousDistance) * gpsDataFreq; // cm/s
            pidAxis->previousDistance = axisDistance;
            pt1FilterUpdateCutoff(&pidAxis->velocityLpf, vaGain);
            const float velocityFiltered = pt1FilterApply(&pidAxis->velocityLpf, velocity);
            ...
            if (ap.sticksActive) {
                ...
                pidAxis->isStopping = true;
                // slowly leak iTerm away
                pidAxis->integral *= iTermLeakGain;
                // rest is handled after axis loop
            } else if (pidAxis->isStopping) {
                ...
                if (velocity * velocityFiltered < 0.0f) {
                    ...
                    pidAxis->isStopping = false;
                }
            }
            pidDA.v[pidAxisIdx] = pidD + pidA;    // save DA here, processed after axis loop
