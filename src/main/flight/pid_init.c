...
#define D_MIN_LOWPASS_HZ 30  // PT1 lowpass cutoff to smooth the boost effect
#define D_MIN_GAIN_FACTOR 0.00015f
#define D_MIN_SETPOINT_GAIN_FACTOR 0.00015f
#endif
...
void pidInitFilters(const pidProfile_t *pidProfile)
{
    ...
    for (int axis = FD_ROLL; axis <= FD_YAW; axis++) {
        pt2FilterInit(&pidRuntime.dMinRange[axis], pt1FilterGain(D_MIN_RANGE_HZ, pidRuntime.dT));
        pt2FilterInit(&pidRuntime.dMinLowpass[axis], pt1FilterGain(D_MIN_LOWPASS_HZ, pidRuntime.dT));
     }
#endif
