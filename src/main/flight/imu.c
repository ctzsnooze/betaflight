...
static float imuCalcGroundspeedGain(float dt)
{
    ...
    // speedBasedGain is 0 at 0.0m/s, 0.04 at 1.0 m/s, 0.16 at 2 m/s, rising slowly towards 1.0 at 5.0 m/s, 2.0 at 10m/s
    ...
    // 3. suppress heading correction unless roll is centered
    // from 1.0 with no Roll to zero more than 12 degrees from flat
    ...
    // 4. attenuate heading correction by pitch angle
...
static void updateGpsHeadingUsable(float groundspeedGain, float imuCourseError, float dt)
{
    ...
    if (!canUseGPSHeading) {
        ...
        canUseGPSHeading = gpsHeadingConfidence > 1.0f;
        // canUseGPSHeading blocks position hold and triggers IMU reorientation in GPS Rescue
    } else {
